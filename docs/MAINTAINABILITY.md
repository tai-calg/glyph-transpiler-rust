...
derived-model facade
    ↓
JSON / Rust / diagram / Studio projections
    ↓
compilation orchestration / CLI / Studio HTTP boundary
```
...
- Studio viewからGlyph source parserへの依存
...
- Glyph Studio 0.4 projection: `glyph/studio_views.py`
...
## 6. Glyph Studio boundaries

Glyph Studioはcompilerとは別の意味解析器ではない。

```text
validated typed design
    ↓
studio_views.py
    display-only orthogonal view model
    ↓
studio.py
    snapshot / artifact / HTTP state
    ↓
studio_ui.py
    browser projection
```

`studio_views.py`は次を行ってよい。

- canonical typed designのgrouping、indexing、表示順序付け
- Protocol eventへのstructured path付与
- symbolic resource identity単位の表示grouping
- verification axis × class matrixの生成

次を行ってはならない。

- Glyph sourceの再parse
- Capability、Resource、Contract意味論の再検証
- Public IRと異なるidentityやContract rowの生成
- Viewごとにcompile pipelineを再実行
- concrete Host runtimeの推測

`StudioSnapshot.glyph04_views`と`studio-views.json`は同じViewModelを使用する。compile error時には最後に成功したViewModelを保持する。

`manual.rs`は`GlyphProjectStudio`が所有するユーザー拡張点である。基底`GlyphStudio`および0.4 View projectionは作成済み`manual.rs`を上書きしない。

詳細は`STUDIO_04_DESIGN.md`に記載する。
...
- Studioの大規模graph layout、cross-view highlight、live runtime traceは未実装である。
...
[ ] Studioはtyped designを再parseしない
[ ] IR → builder → projectionの依存方向を逆転させない
[ ] generated RustとHost Binding traitがrustcを通る
[ ] Studio JavaScriptが構文検査を通る
[ ] manual.rsのユーザー編集を上書きしない
