        ...
        defaults=(
            (
                ...
                read_only_identity_contract(
                    "actuator",
                    "state",
                    source="May UI campaign: reviewed identity actuator",
                ),
            ),
        )
    )

    def may_views(model: object, execution: object) -> dict[str, object]:
        return build_strict_io_state_views(  # type: ignore[arg-type]
            model,
            execution,
            contracts,
            witness_max_cases=64,
        )

    prepare_diagram_app()
    return diagram_app.run_diagram_app(args.input, view_builder=may_views)
