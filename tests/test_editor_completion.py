...
class EditorCompletionTests(unittest.TestCase):
    ...
    def test_lexical_worker_only_promotes_real_top_level_direct_symbols(self) -> None:
        ...
        self.assertIn('if(first==="*")', LEXICAL_WORKER_JS)
        self.assertIn('if(first==="+")', LEXICAL_WORKER_JS)
        self.assertIn('if(first===">"||first==="!"||first==="~"||first==="?")', LEXICAL_WORKER_JS)
        self.assertIn('if(line.startsWith("system"))', LEXICAL_WORKER_JS)
        self.assertIn('if(line.startsWith("machine"))', LEXICAL_WORKER_JS)
        self.assertNotIn("stripComments(source)", LEXICAL_WORKER_JS)
        self.assertNotIn("productRe.exec", LEXICAL_WORKER_JS)
        self.assertNotIn("functionRe.exec", LEXICAL_WORKER_JS)
        self.assertIn("if(type&&sumVariants.has(type))", LEXICAL_WORKER_JS)
    ...
    def test_lexical_worker_extracts_lightweight_symbol_relationships(self) -> None:
        ...
        self.assertIn("machineRows.push", LEXICAL_WORKER_JS)
        self.assertIn("selectorType", LEXICAL_WORKER_JS)
        self.assertIn('if(marker===">")mark(name,"EntryFunction")', LEXICAL_WORKER_JS)
        self.assertIn("const raw=line.match", LEXICAL_WORKER_JS)
        self.assertIn("const ast=line.match", LEXICAL_WORKER_JS)
        self.assertIn('name&&name!=="A"&&name!=="E"', LEXICAL_WORKER_JS)
    ...
    def test_context_classifier_is_bounded_and_glyph_specific(self) -> None:
        ...
        self.assertIn('kind:"bounded-unknown"', CONTEXT_SCRIPT)
        self.assertIn('id:"unsafe-scope"', CONTEXT_SCRIPT)
    ...
    def test_completion_has_context_filtering_accept_revalidation_and_ime_guard(self) -> None:
        ...
        self.assertIn("function finishAcceptance()", COMPLETION_SCRIPT)
        self.assertIn("close:dismiss", COMPLETION_SCRIPT)
        self.assertIn('classification.id==="unsafe-scope"', COMPLETION_SCRIPT)
