...
class DiagramMiddleDragZoomTests(unittest.TestCase):
    def test_identifier_highlight_only_overlays_matching_backgrounds(self) -> None:
        html = enhance_editor_identifier_highlight_html(DIAGRAM_HTML)

        self.assertIn(".identifier-highlight-surface", html)
        self.assertIn("background:transparent", html)
        self.assertIn(".identifier-highlight-layer{", html)
        self.assertIn("color:transparent", html)
        self.assertIn(".identifier-highlight-layer mark{", html)
        self.assertIn("background:rgba(148,163,184,.24)", html)
        self.assertIn(".editor-wrap>.editor", html)
        self.assertNotIn(".editor-wrap.identifier-highlight-active>.editor{", html)
        self.assertNotIn("color:transparent!important", html)
        self.assertNotIn("background:transparent!important", html)

    def test_middle_button_drag_is_translated_into_anchor_preserving_zoom(self) -> None:
    ...
    def test_prepared_app_installs_middle_drag_zoom_after_viewport(self) -> None:
        prepare_diagram_app()
        ...
        self.assertLess(viewport, middle_drag)
        self.assertIn("window.glyphDiagramMiddleDragZoom", html)
