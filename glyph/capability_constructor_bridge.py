...
from .capabilities import CapabilityExtraction, CapabilityModel, _erase_expression, extract_capabilities
...
def _call(text: str) -> tuple[str, tuple[str, ...]] | None:
    ...
    if close_pos != len(value) - 1:
        return None
    return match.group("name"), tuple(item.strip() for item in _split_top_level(value[open_pos + 1 : close_pos], ",") if item.strip())


def _normalize_resolve(expression: str) -> str:
    match = _RESOLVE.fullmatch(expression.strip())
    if match is None:
        return expression
    return f"&{match.group('name')} as share?"


def _unwrap_to_capability_constructor(expression: str, constructor_names: set[str]) -> str:
    normalized = _normalize_resolve(expression)
...
def _analysis_source(source: str, constructor_names: set[str]) -> tuple[str, set[int]]:
    ...
    for index, original in enumerate(lines):
        ...
        if not code[:1].isspace() and stripped.startswith((">", "!", "~")):
            ...
            if equal >= 0:
                ...
                replacement = stripped[: equal + 1] + _unwrap_to_capability_constructor(expression, constructor_names)
        elif code[:1].isspace():
            if ">>" in stripped:
                prefix, expression = stripped.split(">>", 1)
                replacement = prefix + ">>" + _unwrap_to_capability_constructor(expression, constructor_names)
            else:
                binding = stripped.find(":=")
                if binding >= 0:
                    expression = stripped[binding + 2 :]
                    replacement = stripped[: binding + 2] + _unwrap_to_capability_constructor(expression, constructor_names)
                else:
                    replacement = _unwrap_to_capability_constructor(stripped, constructor_names)
        if replacement != stripped:
            indent = code[: len(code) - len(code.lstrip())]
            output[index] = indent + replacement + ((" #" + comment) if marker else "")
            changed.add(index)
    suffix = "\n" if source.endswith("\n") else ""
    return "\n".join(output) + suffix, changed


def _restore_generation_source(source: str, extracted_source: str, changed_lines: set[int]) -> str:
    ...
    for index in changed_lines:
        ...
        if not code[:1].isspace() and stripped.startswith((">", "!", "~")):
            ...
            if original_equal >= 0 and rendered_equal >= 0:
                rendered[index] = rendered[index][: rendered_equal + 1] + _erase_expression(stripped[original_equal + 1 :])
        elif code[:1].isspace():
...
def extract_capabilities_with_constructors(source: str) -> CapabilityExtraction:
    ...
    if constructors:
        ...
        augmented = analysis_source + suffix + "\n".join(item.line for item in constructors) + "\n"
    else:
        augmented = analysis_source
    extracted = extract_capabilities(augmented)
    rendered = _restore_generation_source(source, extracted.source, changed_lines)
    functions = tuple(function for function in extracted.model.functions if not (function.line > original_lines and function.name in constructor_names))
    model = CapabilityModel(
        extracted.model.resources,
        functions,
        extracted.model.aggregates,
        tuple(operation for operation in extracted.model.operations if operation.function not in constructor_names),
    )
