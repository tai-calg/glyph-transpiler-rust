...
def _evaluate_block(
    ...
    states: list[_BlockState] = [_BlockState({}, {})]
    path = (context.entry,)
    for binding in context.block.bindings:
        next_states: list[_BlockState] = []
        for state in states:
            if state.terminated:
                next_states.append(state)
                continue
            site = _TraceSite(context.system, context.entry, binding.line, path)
            if binding.kind == "conditional":
                cases = _evaluate_conditional_binding(binding.source, state, evaluator, site)
            else:
                try:
    ...
    for state in states:
        ...
        cases = evaluator.evaluate(
            ...
            _TraceSite(context.system, context.entry, context.block.final_line, path),
            after_transition=state.transition_calls > 0,
            conditions=state.conditions,
        )
...
def _evaluate_function(
    ...
    cases = evaluator._evaluate_function_decl(
        ...
        identity,
        identity,
        _TraceSite(context.system, context.entry, function.line, (context.entry,)),
        visited=frozenset({function.name}),
        after_transition=False,
        conditions=(),
    )
    return _ContextEvaluation(context, cases)
...
def attach_transition_system_execution_actions(
    ...
    """Attach every execution context, including actionless and unresolved contexts."""
