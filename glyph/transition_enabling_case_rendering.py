...
function lineOf(item,action){const input=inputOf(item),guard=guardOf(item),left=`${input}${guard?`${input?" ":""}[${guard}]`:""}`.trim();return`${left}${action?`${left?" ":""}➞ ${action}`:""}`.trim()}
...
async function apply(){
  if(running)return;
  const stage=document.querySelector(".state-node")?.closest(".graph-stage");if(!stage||stage.dataset.transitionIoClustersReady!=="true")return;
  running=true;
  try{
    const data=await state(),machine=selectedMachine(data);if(!machine)return;
    let changed=0;
    (machine.transitions||[]).forEach((transition,index)=>{const id=transition.id||`T${index+1}`,escaped=window.CSS?.escape?CSS.escape(id):id.replace(/[^A-Za-z0-9_-]/g,"\\$&"),cluster=stage.querySelector(`.transition-io-cluster[data-transition-id="${escaped}"]`);if(cluster&&update(cluster,transition))changed+=1});
    ...
    if(changed||firstReady){
      ...
      window.glyphTransitionSemanticRoleLines?.apply?.();
    }
