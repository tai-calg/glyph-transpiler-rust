...
for(const event of["glyph-state-transition-ir-v3-labels-ready","glyph-state-transition-ir-v2-labels-ready","glyph-transition-input-action-labels-ready","glyph-uml-transition-ready","glyph-locale-changed","glyph-diagram-viewport-change","glyph-execution-context-changed"])document.addEventListener(event,()=>{cache=null;schedule(null,0)});document.addEventListener("pointerup",event=>{if(event.target?.closest?.(".state-node"))schedule(null,40)},true);document.addEventListener("change",event=>{if(event.target?.id==="machine-select"){cache=null;schedule(null,0)}});new MutationObserver(()=>schedule()).observe(document.getElementById("view")||document.body,{childList:true,subtree:true});patchExports();patchReroute();window.glyphTransitionIoClusters={render:async()=>{cache=null;for(let attempt=0;attempt<100&&running;attempt+=1)await new Promise(resolve=>setTimeout(resolve,10));if(running)throw Error("transition I/O renderer did not become idle");return render()},maxDistance:MAX_DISTANCE,version:2};schedule(null,0);
})();
</script>
"""
