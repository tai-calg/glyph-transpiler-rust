...
function inputOf(transition){const trigger=triggerOf(transition),unknown=unknownOf(transition);if(trigger)return`${trigger.role==="provisional-trigger"?"? ":""}${trigger.display}`;if(unknown.length)return`? ${unknown.join(" & ")}`;return"otherwise"}
function projectionOf(transition){return window.GlyphExecutionContext?.projectionFor?.(transition)||{action:transition?.action,status:"auto"}}
function actionOf(transition){const action=projectionOf(transition).action;if(typeof action==="string")return text(action);return text(action?.display)||text(action?.expression)}
function ioOf(transition){const input=inputOf(transition),guard=guardsOf(transition).join(" & "),projection=projectionOf(transition);if(projection.status==="missing")return`${input}${guard?` [${guard}]`:""} [${both("選択した実行コンテキストでは到達不能","unreachable in selected execution context")}]`;const action=typeof projection.action==="string"?text(projection.action):text(projection.action?.display)||text(projection.action?.expression);return`${input}${guard?` [${guard}]`:""}${action?` ➞ ${action}`:""}`}
...
function signatureOf(machine){return[window.GlyphI18n?.locale||document.documentElement.lang||"ja",window.GlyphExecutionContext?.signature?.()||"auto",machine?.name||"",...(machine?.transitions||[]).map(item=>[item.id||"",JSON.stringify(item.trigger||null),JSON.stringify(item.guards||[]),JSON.stringify(item.unclassified_conditions||[]),actionOf(item)].join("\u001f"))].join("\u001e")}
...
for(const event of["glyph-state-transition-ir-v3-labels-ready","glyph-state-transition-ir-v2-labels-ready","glyph-transition-input-action-labels-ready","glyph-uml-transition-ready","glyph-locale-changed","glyph-diagram-viewport-change","glyph-execution-context-changed"])document.addEventListener(event,()=>{cache=null;schedule(null,0)});document.addEventListener("pointerup",event=>{if(event.target?.closest?.(".state-node"))schedule(null,40)},true);document.addEventListener("click",event=>{if(!event.target?.closest?.(".transition-io-cluster"))select(null)});document.addEventListener("change",event=>{if(event.target?.id==="machine-select"){cache=null;schedule(null,0)}});new MutationObserver(()=>schedule()).observe(document.getElementById("view")||document.body,{childList:true,subtree:true});patchExports();patchReroute();window.glyphTransitionIoClusters={render:async()=>{cache=null;for(let attempt=0;attempt<100&&running;attempt+=1)await new Promise(resolve=>setTimeout(resolve,10));if(running)throw Error("transition I/O renderer did not become idle");return render()},maxDistance:MAX_DISTANCE,version:2};schedule(null,0);
})();
</script>
"""
