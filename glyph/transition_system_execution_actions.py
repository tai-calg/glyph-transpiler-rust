...
class _SystemExecutionEvaluator:
    ...
    Operations before the machine transition are input/preparation work and are not
    attached to an edge. Every proven external operation after the transition is a
    System Action, whether it consumes the returned state or is merely sequenced by
    ...
    def evaluate(
        ...
        *,
        ...
        argument_results: list[_Evaluation] = []
        active = after_transition
        for symbolic_argument, concrete_argument in zip(
            ...
            active = active or result.transition_calls > 0

        symbolic_call = CallExpr(
            symbolic.callee,
            tuple(item.value.symbolic for item in argument_results),
        )
                ...
                invocations.append(
                    {
                        ...
                        "dataflow_path": (
                            [*site.path, name] if result_dependent else []
                        ),
                    }
                )
            return _Evaluation(
        ...
        if name in visited or len(function.params) != len(argument_results):
            return _Evaluation(
                base.value,
                base.invocations,
                base.unresolved
                or _contains_marker(symbolic_call)
                or sequenced_after_transition,
                base.transition_calls,
            )
        ...
        if function.expression is not None:
            nested = self.evaluate(
                ...
                after_transition=nested_after_transition,
            )
            return _Evaluation(
                nested.value,
...
def _evaluate_block(
    ...
    for binding in context.block.bindings:
        ...
        after_transition = after_transition or result.transition_calls > 0
...
def _binding(evaluation: _ContextEvaluation) -> dict[str, object] | None:
    ...
    result_dependent = sum(
        ...
        if item.get("execution_relation") == "result-dependency"
    )
    sequenced = sum(
        1
        for item in invocations
    ...
    return {
        ...
        # Compatibility for consumers of the former result-only pass.
...
def attach_transition_system_execution_actions(
    ...
    """Attach every proven post-transition system operation to its entry context."""
    ...
    for original in result.get("transitions", []):
        ...
        for context in contexts:
            ...
            if evaluation.unresolved:
                ...
                _append_diagnostic_once(
                    ...
                    _diagnostic(
                        _UNRESOLVED_CODE,
                        (
                            f"{context_name} executes `{branch_context.next_function}` "
                            "through a path whose post-transition operations cannot be proven"
                        ),
                        line,
                    ),
                )
            ...
            if binding is not None:
                ...
                binding_count += 1
                for invocation in binding["action_invocations"]:
...
# Compatibility name for callers of the former result-consumer-only pass.
