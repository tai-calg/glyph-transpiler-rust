...
function arrange(stage){
 ...
 labels.forEach((label,index)=>{
   const id=labelId(label,index),anchor=anchorFor(label,index,stage),manual=saved[id];
   ...
   let preferred=manual&&Number.isFinite(manual.x)&&Number.isFinite(manual.y)?project({x:manual.x,y:manual.y},anchor):anchor;
   let selectedPosition=choose(label,anchor,preferred,stage,obstacles,placed);
   if((!selectedPosition||selectedPosition.collisions>0)&&!label.classList.contains("compact")){compact(label,id);selectedPosition=choose(label,anchor,preferred,stage,obstacles,placed)}
   if(!selectedPosition)return;
   label.style.left=`${selectedPosition.point.x}px`;label.style.top=`${selectedPosition.point.y}px`;label.dataset.manualLabel=manual?"true":"false";label.dataset.labelDistance=String(Math.hypot(selectedPosition.point.x-anchor.x,selectedPosition.point.y-anchor.y));label.classList.toggle("layout-constrained",selectedPosition.collisions>0);placed.push(selectedPosition.rect);
 });
 stage.dataset.labelEditorReady="true";stage.dataset.labelMaxDistance=String(MAX_DISTANCE);
}
function schedule(stage,delay=24){clearTimeout(timer);timer=setTimeout(()=>{state().then(()=>arrange(stage||document.querySelector(".graph-stage"))).catch(()=>{})},delay)}
function bind(label,stage,index){
 ...
 label.addEventListener("pointermove",event=>{if(!drag||drag.label!==label)return;event.preventDefault();const desired={x:drag.left+event.clientX-drag.startX,y:drag.top+event.clientY-drag.startY},point=project(desired,drag.anchor);label.style.left=`${clamp(point.x,label.offsetWidth/2+8,stage.scrollWidth-label.offsetWidth/2-8)}px`;label.style.top=`${clamp(point.y,label.offsetHeight/2+8,stage.scrollHeight-label.offsetHeight/2-8)}px`});
 label.addEventListener("pointerup",event=>{if(!drag||drag.label!==label)return;event.preventDefault();event.stopPropagation();label.classList.remove("dragging-label");const saved=read(stage),anchor=anchorFor(label,index,stage),point=project({x:num(label.style.left),y:num(label.style.top)},anchor);saved[id]=point;write(stage,saved);label.dataset.manualLabel="true";drag=null;schedule(stage,0)});
...
document.addEventListener("pointerup",event=>{const node=event.target?.closest?.(NODE_SELECTOR);if(node){const stage=node.closest(".graph-stage");schedule(stage,40);setTimeout(()=>schedule(stage,0),140)}},true);
