...
function arrange(stage){
 ...
 labels.forEach((label,index)=>{
   ...
   if(manual&&Number.isFinite(manual.x)&&Number.isFinite(manual.y)){
     const point=constrain(project({x:manual.x,y:manual.y},anchor),label,stage);place(label,point,anchor,true,obstacles,placed);return;
   }
...
function bind(label,stage,index){
 ...
 label.addEventListener("pointerup",event=>{if(!drag||drag.label!==label)return;event.preventDefault();event.stopPropagation();label.classList.remove("dragging-label");const saved=read(stage),anchor=anchorFor(label,index,stage),point=constrain(project({x:num(label.style.left),y:num(label.style.top)},anchor),label,stage);saved[id]=point;write(stage,saved);label.dataset.manualLabel="true";drag=null;schedule(stage,0)});
