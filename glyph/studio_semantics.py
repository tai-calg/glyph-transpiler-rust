...
def application_target_entity_id(target_kind: str, target: str) -> str:
    ...
    if normalized in {"function", "pure-function", "effect-function"}:
        return function_entity_id(target)
    if normalized == "resource":
        return resource_entity_id(target)
    if normalized in {"type", "product", "sum", "aggregate"}:
        return aggregate_entity_id(target)
    if normalized in {"field", "place"}:
...
class _IndexBuilder:
    def __init__(self) -> None:
        ...
        self._relations: dict[tuple[str, str, str, int | None], dict[str, object]] = {}
    ...
    def entity(
        ...
        *,
        ...
        if existing.get("line") is None and line is not None:
            existing["line"] = line
        view_list = existing["views"]
        alias_list = existing["aliases"]
        detail_map = existing["details"]
    ...
    def relation(
        ...
        *,
        ...
        if source not in self._entities or target not in self._entities:
            raise ValueError(f"semantic relation endpoint is undefined: {source} {kind} {target}")
...
def _type_target(builder: _IndexBuilder, type_value: Mapping[str, Any], *, view: str) -> str:
    name = _text(type_value.get("name"), "unknown")
    entity_id = resource_entity_id(name) if type_value.get("state") else type_entity_id(name)
    builder.entity(
        entity_id,
        "resource" if type_value.get("state") else "type",
        name,
        views=(view,),
...
def _subject_entity(
    ...
    if axis == "handler" and "." in subject:
        ...
        direct = known.get(f"handler-step:{handler_name}:{operation}:{line or 0}")
        if direct:
            return direct
        direct = known.get(f"handler-step:{handler_name}:{operation}")
        if direct:
            return direct
    entity_id = semantic_id("subject", f"{axis}:{subject}")
    builder.entity(entity_id, "subject", subject, line=line, views=("Verification",), details={"axis": axis})
    return entity_id
...
def build_semantic_index(
    ...
    for aggregate in _records(capability.get("aggregates")):
        ...
        builder.entity(entity_id, "aggregate", name, line=_line(aggregate.get("line")), views=("Capability",))
        known[name] = entity_id
        for index, member in enumerate(_records(aggregate.get("members"))):
            target = _type_target(builder, _mapping(member.get("type", member)), view="Capability")
            builder.relation(
                entity_id,
                "stores",
                target,
    ...
    for function in _records(capability.get("functions")):
        ...
        for parameter in _records(function.get("params")):
            place_name = _text(parameter.get("name"))
            place_id = _text(parameter.get("entity_id"), place_entity_id(name, f"param:{place_name}"))
            ...
            builder.entity(
                ...
                views=("Capability", "Resource"),
                details={"role": "parameter", "type": type_value.get("raw") or type_value.get("name")},
            )
            target = _type_target(builder, type_value, view="Capability")
            builder.relation(function_id, "accepts", place_id, line=_line(parameter.get("line")), views=("Capability",))
            builder.relation(
                place_id,
            ...
            known[f"place:{name}:{place_name}"] = place_id

        result = _mapping(function.get("result"))
        result_place = _text(function.get("result_entity_id"), place_entity_id(name, "result"))
        builder.entity(
            result_place,
            "place",
            f"{name}.result",
            line=_line(function.get("line")),
            views=("Capability", "Resource"),
            details={"role": "result", "type": result.get("raw") or result.get("name")},
        )
        result_target = _type_target(builder, result, view="Capability")
        builder.relation(function_id, "returns", result_place, line=_line(function.get("line")), views=("Capability",))
        builder.relation(
            result_place,
    ...
    for operation in _records(capability.get("operations")):
        operation_id = _text(operation.get("entity_id"), capability_operation_entity_id(operation))
        ...
        if function_id is None:
            ...
            builder.entity(function_id, "function", function_name, line=_line(operation.get("line")), views=("Capability",))
        ...
        builder.relation(function_id, relation_kind, operation_id, line=_line(operation.get("line")), views=("Capability",))

    resource_view = _mapping(views.get("resource"))
    for identity in _records(resource_view.get("identities")):
        raw_identity = _text(identity.get("identity"))
        identity_id = _text(identity.get("entity_id"), identity_entity_id(raw_identity))
        resource_name = _text(identity.get("resource"))
        resource_id = known.get(resource_name, resource_entity_id(resource_name))
        builder.entity(resource_id, "resource", resource_name, views=("Capability", "Resource"))
        builder.entity(
            identity_id,
            "resource-identity",
            ...
            details={"identity": raw_identity, "states": identity.get("states", []), "capabilities": identity.get("capabilities", [])},
        )
        builder.relation(identity_id, "instance-of", resource_id, line=_line(identity.get("line")), views=("Resource",))
        known[raw_identity] = identity_id
        for transition in _records(identity.get("transitions")):
            transition_id = _text(transition.get("entity_id"), resource_transition_entity_id({**transition, "identity": raw_identity}))
            function_name = _text(transition.get("function"))
            function_id = known.get(function_name, function_entity_id(function_name))
            builder.entity(function_id, "function", function_name, line=_line(transition.get("line")), views=("Resource", "Capability"))
            builder.entity(
                transition_id,
                "resource-transition",
                ...
                views=("Resource",),
                details={"source": transition.get("source"), "target": transition.get("target"), "kind": transition.get("kind")},
            )
            relation_kind = {
                "create": "creates",
                "preserve": "preserves",
            ...
            builder.relation(function_id, relation_kind, identity_id, line=_line(transition.get("line")), views=("Resource", "Capability"), details={"transition": transition_id})
            builder.relation(transition_id, "instance-of", identity_id, line=_line(transition.get("line")), views=("Resource",))
    ...
    for world in _records(_mapping(views.get("world_region")).get("worlds")):
        ...
        builder.entity(entity_id, "world", name, line=_line(world.get("line")), views=("World/Region", "Verification"), details={"locus": world.get("locus"), "region": world.get("region", [])})
        known[f"world:{name}"] = entity_id
        known[name] = known.get(name, entity_id)
    ...
    for protocol in _records(_mapping(views.get("protocol")).get("protocols")):
        ...
        builder.entity(protocol_id, "protocol", name, line=_line(protocol.get("line")), views=("Protocol", "Verification"))
        known[f"protocol:{name}"] = protocol_id
        known[name] = known.get(name, protocol_id)
        for event in _records(protocol.get("events")):
            path = _text(event.get("path"), "root")
            event_id = _text(event.get("entity_id"), protocol_event_entity_id(name, path))
            direction = _text(event.get("direction"))
            message_type = _text(event.get("type"), "unknown")
            builder.entity(event_id, "protocol-event", f"{direction} {message_type}", line=_line(protocol.get("line")), views=("Protocol",), details={"path": path, "controls": event.get("controls", [])})
            builder.relation(protocol_id, "contains", event_id, line=_line(protocol.get("line")), views=("Protocol",))
            type_id = type_entity_id(message_type)
            builder.entity(type_id, "type", message_type, views=("Protocol", "Capability"))
            builder.relation(event_id, "sends" if direction == "send" else "receives", type_id, line=_line(protocol.get("line")), views=("Protocol",))
    ...
    for handler in _records(_mapping(views.get("handler")).get("handlers")):
        ...
        builder.entity(handler_id, "handler", name, line=_line(handler.get("line")), views=("Handler", "Verification"))
        known[f"handler:{name}"] = handler_id
        known[name] = known.get(name, handler_id)
        previous_node: str | None = None
        for node in _records(handler.get("nodes")):
            node_id = _text(node.get("entity_id"), handler_node_entity_id(name, _text(node.get("id"))))
            builder.entity(node_id, "handler-node", _text(node.get("label")), line=_line(node.get("line")), views=("Handler",), details={"node_kind": node.get("kind"), "arguments": node.get("arguments", []), "verification": node.get("verification")})
            builder.relation(handler_id, "contains", node_id, line=_line(node.get("line")) or _line(handler.get("line")), views=("Handler",))
            if previous_node is not None:
                builder.relation(previous_node, "next", node_id, line=_line(node.get("line")), views=("Handler",))
            previous_node = node_id
            operation = _text(node.get("label"))
            if operation:
                known.setdefault(f"handler-step:{name}:{operation}", node_id)
                known[f"handler-step:{name}:{operation}:{_line(node.get('line')) or 0}"] = node_id
    ...
    for law in _records(_mapping(views.get("law")).get("laws")):
        ...
        builder.entity(law_id, "law", name, line=_line(law.get("line")), views=("Law/Monitor", "Verification"), details={"formula": law.get("formula", {}), "verification": law.get("verification")})
        known[f"law:{name}"] = law_id
        known[name] = known.get(name, law_id)
    ...
    for application in applications:
        ...
        builder.entity(target_id, application_target_entity_kind(target_kind), target, line=_line(application.get("line")), views=("Capability", "World/Region", "Protocol", "Handler", "Law/Monitor"))
        ...
        for axis, selected, relation_kind in selections:
            ...
            builder.entity(contract_id, axis, selected, views=({"world": "World/Region", "protocol": "Protocol", "handler": "Handler"}[axis],))
            builder.relation(contract_id, "applies", target_id, line=_line(application.get("line")), views=("World/Region", "Protocol", "Handler"))
            builder.relation(target_id, relation_kind, contract_id, line=_line(application.get("line")), views=("World/Region", "Protocol", "Handler"))
        laws = row.get("laws") if isinstance(row.get("laws"), list) else []
        for law_name in (item for item in laws if isinstance(item, str) and item):
            law_id = law_entity_id(law_name)
            builder.entity(law_id, "law", law_name, views=("Law/Monitor",))
            builder.relation(law_id, "applies", target_id, line=_line(application.get("line")), views=("Law/Monitor",))
            builder.relation(target_id, "constrained-by", law_id, line=_line(application.get("line")), views=("Law/Monitor",))

    host = _mapping(design.get("host_requirements"))
    for requirement in _records(host.get("operations")):
        requirement_id = host_requirement_entity_id(_text(requirement.get("id"), "unknown"))
        builder.entity(requirement_id, "host-requirement", _text(requirement.get("kind"), "Host requirement"), line=_line(requirement.get("line")), views=("Handler", "Law/Monitor", "Verification"), aliases=(_text(requirement.get("subject")),), details=dict(requirement))
        contract = _text(requirement.get("contract"))
        if contract:
            candidates = [known.get(f"handler:{contract}"), known.get(f"law:{contract}"), known.get(f"protocol:{contract}"), known.get(f"world:{contract}")]
            owner = next((item for item in candidates if item), None)
            if owner:
                builder.relation(owner, "requires-host", requirement_id, line=_line(requirement.get("line")), views=("Handler", "Law/Monitor", "Verification"))

    verification = _mapping(views.get("verification_strength"))
    for item in _records(verification.get("items")):
        verification_id = _text(item.get("entity_id"), verification_entity_id(item))
        axis = _text(item.get("axis"), "unknown")
        subject = _text(item.get("subject"), "subject")
        builder.entity(verification_id, "verification", subject, line=_line(item.get("line")), views=("Verification",), details={"axis": axis, "classes": item.get("classes", []), "statement": item.get("statement")})
        subject_id = _subject_entity(builder, axis, subject, _line(item.get("line")), known)
        builder.relation(subject_id, "verified-by", verification_id, line=_line(item.get("line")), views=("Verification",))
