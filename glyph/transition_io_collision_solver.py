...
.transition-io-cluster.nano-io .transition-io-value{font-size:5px;line-height:1}
</style>
"""
...
const RINGS=[0,12,24,36,48,60,72,84,96],ANGLES=72,PER_MODE_LIMIT=72,SEARCH_MS=1600,MAX_RETRIES=4;
...
document.addEventListener("glyph-transition-io-clusters-ready",()=>schedule(null,0));document.addEventListener("pointerup",event=>{if(event.target?.closest?.(".state-node")){delete event.target.closest(".graph-stage")?.dataset.transitionIoRetry;schedule(null,0);setTimeout(()=>schedule(null,0),140)}},true);document.addEventListener("glyph-diagram-viewport-change",()=>schedule(null,0));window.addEventListener("resize",()=>schedule(null,30));window.glyphTransitionIoCollisionSolver={run:()=>schedule(null,0),maxDistance:MAX_DISTANCE};schedule(null,0);
})();
</script>
"""
