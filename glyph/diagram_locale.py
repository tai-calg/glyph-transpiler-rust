...
function installOverrides(){
 if(typeof renderDiagnostics==="function"&&!renderDiagnostics.__glyphLocalized){const localized=function(){const rows=snapshot?.diagnostics||[];diagnostics.innerHTML=rows.map(diagnosticMarkup).join("")};localized.__glyphLocalized=true;renderDiagnostics=localized}
 if(typeof renderMachineDiagnostics==="function"&&!renderMachineDiagnostics.__glyphLocalized){const localized=function(machine){const rows=machine?.diagnostics||[];if(!rows.length)return "";return `<section class="analysis-panel"><div class="analysis-title">${html(t("warnings"))} · ${rows.length}</div>${rows.map(item=>`<div class="analysis-item" data-line="${Number(item?.line||0)}" title="${html(String(item?.message??""))}"><span class="analysis-code">${html(item?.code||"warning")}</span><span class="analysis-message">${html(messageOf(item))}${helpOf(item)?`<span class="analysis-help">${html(helpOf(item))}</span>`:""}</span><span class="analysis-line">L${html(item?.line||"?")}</span></div>`).join("")}</section>`};localized.__glyphLocalized=true;renderMachineDiagnostics=localized}
 if(typeof renderSummary==="function"&&!renderSummary.__glyphLocalized){const localized=function(){const s=snapshot?.views?.summary||{};document.getElementById("summary").innerHTML=[["systems",s.systems,""],["callables",s.callables,""],["types",s.types,""],["machines",s.machines,""],["warnings",s.state_warnings,"warn"]].map(([key,value,cls])=>`<span class="pill ${cls}">${html(t(key))}: ${value??0}</span>`).join("")};localized.__glyphLocalized=true;renderSummary=localized}
...
function apply(){
 ...
 const tabs=document.querySelectorAll(".tab");tabs.forEach(tab=>{tab.textContent=tab.dataset.tab==="state"?t("state"):t("io")});
 const status=document.getElementById("status");if(status){const key=["starting","ready","busy","error"].find(item=>status.classList.contains(item)||status.textContent.trim().toLowerCase()===item);if(key)status.textContent=t(key)}
 const title=document.querySelector(".view-controls h2");if(title)title.textContent=document.querySelector('.tab.active')?.dataset.tab==="state"?t("stateTitle"):t("ioTitle");
 const note=document.querySelector(".view-controls .note");if(note)note.textContent=document.querySelector('.tab.active')?.dataset.tab==="state"?t("stateNote"):t("ioNote");
 ...
 const legend=document.querySelectorAll(".legend span");if(legend[0])legend[0].textContent=t("reachable");if(legend[1])legend[1].textContent=t("unreachable");
 const theme=document.getElementById("diagram-theme");if(theme){const options=theme.options;if(options[0])options[0].textContent=t("themeWhite");if(options[1])options[1].textContent=t("themeMono")}
