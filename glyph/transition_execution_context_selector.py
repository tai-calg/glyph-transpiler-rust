...
function actionFor(transition){
  if(currentKey===MACHINE)return transition?.machine_action||null;
  if(currentKey.startsWith("context:")){
    const binding=bindingFor(transition,currentKey);
    return composedAction(transition?.machine_action,binding?.action,binding);
  }
  return transition?.display_action||transition?.action||null;
}
function optionLabel(context){
  if(context.system&&context.entry)return`${context.system} / ${context.entry}`;
  return context.entry||context.system||"implicit caller";
}
function restore(machine,valid){
...
function ensureControl(machine){
  ...
  const contexts=contextsFor(machine),valid=new Set([AUTO,MACHINE,...contexts.map(item=>item.key)]);
  let control=document.getElementById("execution-context-control");
  if(!contexts.length){control?.remove();currentKey=AUTO;return false}
  if(!control){
    control=document.createElement("div");control.id="execution-context-control";control.className="execution-context-control";
    const label=document.createElement("label");label.htmlFor="execution-context-select";label.textContent="Execution";
  ...
  const next=currentMachine?.name===machine?.name&&valid.has(currentKey)?currentKey:restore(machine,valid);
  currentMachine=machine;currentKey=next;select.value=currentKey;
  select.onchange=()=>{currentKey=valid.has(select.value)?select.value:AUTO;sessionStorage.setItem(storageKey(machine),currentKey);publish()};
...
for(const event of["glyph-state-transition-ir-v3-labels-ready","glyph-transition-io-clusters-ready","glyph-locale-changed"]){document.addEventListener(event,()=>{cache=null;schedule(0)})}
