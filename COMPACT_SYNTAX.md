...
型位置の `/` は結果型の区切りだけを表す。式位置の `?` は失敗伝播だけを表すため、一つの記号に複数の概念を持たせない。従来の `R<T,E>` も引き続き使用できる。旧候補だった `T?E` は受理しない。

展開では行数を変えないため、構文エラーの行番号は元の`.glyph`ファイルと一致する。型の一文字短縮は型位置でのみ有効で、同名のユーザー定義型が存在する場合はユーザー定義型を優先する。

ガード関数のフォールバックは短縮しない。最後の分岐も必ず `_=>expression` と明示し、残りすべての入力を受けることをソース上で可視化する。
